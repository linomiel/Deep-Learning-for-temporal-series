...
if not opt then
	...
	cmd:option('-save', "", 'the subsubdirectory where the final NN is.')
	cmd:text()
end
...
here = lfs.currentdir()
if not lfs.chdir(here..'/'..opt.directory ..'/' .. opt.subdirectory ..'/'.. opt.save) then--change the working directory to the model directory
	error('fail in directory change')
end

model = torch.load('model2.data', ascii)
...
for k = 1, 5 do
	...
	for i = 1, eoc do
		local temp = model:forward(current_class[i]:narrow(1,2,current_class:size(2) - 1))
		temp = temp:reshape(temp:size(1)*temp:size(2))
